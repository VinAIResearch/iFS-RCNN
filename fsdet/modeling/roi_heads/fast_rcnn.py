...
COCO_BASE_INDICES = sorted(set(range(81)) - set([0, 1, 2, 3, 4, 5, 6, 8, 14, 15, 16, 17, 18, 19, 39, 56, 57, 58, 60, 62]))
LVIS_BASE_INDICES = sorted(set(range(1230)) - set([
...
def fast_rcnn_inference(
    boxes, scores, image_shapes, score_thresh, nms_thresh, topk_per_image, use_nms
):
    """
...
def fast_rcnn_inference_single_image(
    boxes, scores, image_shape, score_thresh, nms_thresh, topk_per_image, use_nms
):
    """
...
class FastRCNNOutputs(object):
    ...
    def predict_boxes_iou(self):
        ...
        if self.pred_proposal_deltas.dim() == 3:
            self.fg_proposal_uncertainty = F.softplus(self.fg_proposal_deltas[..., 1]) # best for both training and finetuning
