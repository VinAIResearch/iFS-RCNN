...
def parse_args():
    ...
    # Paths
    parser.add_argument("--src1", type=str, default="", help="Path to the main checkpoint")
    parser.add_argument("--src2", type=str, default="", help="Path to the secondary checkpoint (for combining)")
    ...
    # Targets
    parser.add_argument("--param-name", type=str, nargs="+", default=[], help="Target parameter names")
...
def ckpt_surgery(args):
    ...
    def surgery(param_name, is_weight, tar_size, ckpt, ckpt2=None):
        ...
        if is_weight:
            ...
            if "mask_head" in param_name or "cate_pred" in param_name:
        ...
        if args.coco or args.lvis:
            for i, c in enumerate(BASE_CLASSES):
                ...
                if "cls_score" in param_name or "mask_head" in param_name or "cate_pred" in param_name:
        ...
        if args.coco or args.lvis:
            for i, c in enumerate(NOVEL_CLASSES):
                if "cls_score" in param_name or "mask_head" in param_name or "cate_pred" in param_name:
                    ...
                    new_weight[IDMAP[c] * 4 : (IDMAP[c] + 1) * 4] = ckpt2_weight[i * 4 : (i + 1) * 4]
            if "cls_score" in param_name:
...
def surgery_loop(args, surgery):
    ...
    tar_size_dict = {
        ...
        "roi_heads.box_predictor.bbox_pred": TAR_SIZE * 4,
        "ins_head.cate_pred": TAR_SIZE,
    }
