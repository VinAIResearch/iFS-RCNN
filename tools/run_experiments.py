...
def parse_args():
    ...
    parser.add_argument(
        "--shots", type=int, nargs="+", default=[1, 2, 3, 5, 10, 30], help="Shots to run experiments over"
    )
    parser.add_argument("--seeds", type=int, nargs="+", default=[1, 10], help="Range of seeds to run")
    ...
    # Model
    parser.add_argument("--fc", action="store_true", help="Model uses FC instead of cosine")
    parser.add_argument("--two-stage", action="store_true", help="Two-stage fine-tuning")
    parser.add_argument("--novel-finetune", action="store_true", help="Fine-tune novel weights first")
    parser.add_argument("--unfreeze", action="store_true", help="Unfreeze feature extractor")
    # PASCAL arguments
...
def run_exp(cfg, configs, seed, shot):
    ...
    if "test_all" in output_dir:
        ...
        test_cmd = (
            f'python3 -m tools.ckpt_surgery {"--coco" if args.coco else ""} '
        ...
        test_cmd = (
            f"python3 -m tools.test_net --num-gpus {args.num_gpus} "
    ...
    if not os.path.exists(model_path):
        train_cmd = (
            f"python3 -m tools.train_net --dist-url auto --num-gpus {args.num_gpus} "
    ...
    if not os.path.exists(res_path):
        test_cmd = (
            f"python3 -m tools.test_net --dist-url auto --num-gpus {args.num_gpus} "
