        ...
        if args.coco or args.lvis:
            for i, c in enumerate(BASE_CLASSES):
                ...
                if "cls_score" in param_name or "mask_head" in param_name:
                    ...
                    new_weight[IDMAP[c] * 4 * factor : (IDMAP[c] + 1) * 4 * factor] = pretrained_weight[
                        idx * 4 * factor : (idx + 1) * 4 * factor
                    ]
        else:
        ...
        if args.coco or args.lvis:
            for i, c in enumerate(NOVEL_CLASSES):
                if "cls_score" in param_name or "mask_head" in param_name:
                    ...
                    new_weight[IDMAP[c] * 4 * factor : (IDMAP[c] + 1) * 4 * factor] = ckpt2_weight[
                        i * 4 * factor : (i + 1) * 4 * factor
                    ]
            if "cls_score" in param_name:
...
if __name__ == "__main__":
    ...
    if "mask" in args.src1:
        args.param_name += ["roi_heads.mask_head.predictor"]
