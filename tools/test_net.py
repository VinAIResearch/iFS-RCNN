    ...
    def test(self, ckpt):
        ...
        if comm.is_main_process():
            ...
            if (self.best_res is None) or (
                self.best_res is not None and self.best_res["bbox"]["AP"] < res["bbox"]["AP"]
            ):
                self.best_res = res
                self.best_file = ckpt
...
def main(args):
    ...
    if args.eval_only:
        ...
        if args.eval_iter != -1:
            ...
            ckpt_file = os.path.join(cfg.OUTPUT_DIR, "model_{:07d}.pth".format(args.eval_iter - 1))
            # resume = False
        else:
...
if __name__ == "__main__":
    ...
    if args.eval_during_train or args.eval_all:
        args.dist_url = "tcp://127.0.0.1:{:05d}".format(np.random.choice(np.arange(0, 65534)))
